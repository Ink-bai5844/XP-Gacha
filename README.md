...
1. 完整解压 `XP-Gacha-v0.2.8-portable-win64.zip`，不要在压缩软件预览窗口中直接运行。
...
后端只允许执行 25 个白名单任务，同一时间最多运行一个任务。任务日志使用绝对游标增量传输，后端与脚本输出框均只保留最新 500 行，较早内容会显示为已截断。采集任务使用协作式安全中止，危险操作要求显式勾选确认。
...
采集器不依赖 Web 前端。四种流程使用同一套参数、断点和结构化失败记录；在线信息只有在 CSV 元数据与缩略图都存在时才算完成。NH/JM 在线首轮会流式执行：每个列表页一解析出项目就立即进入详情与封面队列，列表发现和项目采集互相重叠，不再等待所有列表页遍历完。新采集到的元数据行会立即追加并刷新到 CSV，后续轮次只处理上一轮未完成的列表页或项目。

`--workers` 表示并发上限，也是起始并发数。采集器只按完整的连续 30 秒窗口统计成功抓取请求数：任务启动后的第一个完整窗口仅用于预热并直接丢弃；此后同一并发档至少采集两个完整窗口。若两窗成功数中的较大值至少为 `10`，且两者相差不超过 `15%` 或绝对值不超过 `2`，则以两窗平均值作为该档稳定得分；若两窗都低于 `10`，或两窗波动较大，则再采集第三个完整窗口并取三窗中位数。只有得到稳定得分后才会探测相邻并发档位，并以相同流程评价新档位。超时、429 和其他限流失败数量、比例均不参与并发决策，目标只是在不超过输入上限的前提下尽量提高稳定的单位时间成功数。Web 界面的独立进度区会实时显示轮次、阶段、列表页、完全成功/缺信息/缺图/全缺、进度百分比、CSV 写入数、当前并发，以及当前/前两个 30 秒窗口的成功吞吐和“预热 / 稳定采样 / 相邻档探测”状态。

`--max-rounds 0` 是默认值，表示持续重试到全部完成。终端按 `Ctrl+C`，或 Web 点击“安全保存并停止”，都会停止提交新请求，等待已经在执行的请求返回，刷新并关闭 CSV，再原子压缩 JSONL 断点后退出；下次使用相同参数即可继续。由于不会强杀请求，中止等待时间最多可能接近表单中的“单次请求超时秒数”，中止期间不要关闭后端进程。
...
先查看 `BUILD-INFO.json`，确认使用的是当前 `v0.2.8`，不是旧的 `v0.2.0` 启动器。`v0.2.1` 及以后版本已加入 MySQL 8.4 公钥认证参数。
